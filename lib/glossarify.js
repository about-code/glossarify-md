...
function prepare(context) {
    ...
    // Internally use a glossaries map object with its key being the glossary
    // file path. This allows to reconstruct the key from the vFile metadata
    // given when proccesing the glossary file that config belongs to (see
    // terminator()).
    ...
    return new Promise((resolve, reject) => {
        unifiedNgin({
            ...
            ,silent: false
                ...
                if(err) {path.resolve(baseDir, file.path)
                    reject(err);path.resolve(baseDir, file.path)
                } else {path.resolve(baseDir, file.path)
                    context.vFiles = [...context.vFilepath.resolve(baseDir, file.path)s, ...uContext.files];
                    resolve(context);
                }
            }
        )
