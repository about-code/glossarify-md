...
function visitNode(node, terms, glossary) {
    if (node.type === "heading" && node.depth > 1) {
        ...
        const term = terms[0];
        const longDesc = node.value;
        const shortDesc = longDesc.match(/(.*\. )/);
        term.shortDesc = shortDesc ? shortDesc[1].trim() : "";
        term.longDesc = longDesc
        State.next(State.AWAIT_NEW_TERM);
...
function writeTestOutput(context) {
    ...
    return new Promise((resolve, reject) => {
        fs.outputFile(termsFile, JSON.stringify(context.terms, null, 2), (err) => {
