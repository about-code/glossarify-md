...
/**
 * Establishes an order in two dimensions. In the first dimension terms are
 * sorted by their text value. If these are equal they are sorted by the path
 * to the file they were declared in. It's important that a nodes `file`
 * property is a complete file path. It's likely there will be projects with
 * multiple glossaries in different folders but all named 'glossary.md' so a
 * (v)file's "basename" is not sufficient.
 *
 * @static
 * @param {Term} t1
 * @param {Term} t2
 */
Term.compare = function(t1, t2) {
    let primaryOrder = collator.compare(t1.value, t2.value);
    if (primaryOrder !== 0) {
        return primaryOrder;
    } else {
        return collator.compare(t1.file, t2.file);
    }
};
