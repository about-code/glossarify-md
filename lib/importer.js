...
function skosToGlossaryObj(ldNodes) {
    const conceptScheme = ldNodes.find(ldNode => ldValue(ldNode[JsonLd["@type"]]) === Skos.ConceptScheme) || {};
    const glossaryObj = new ExportTypeGlossary({
        // Map skos:ConceptScheme onto our 'Glossary' import format type
        type: "Glossary"
        ,uri: ldValue(conceptScheme[JsonLd["@id"]])
        ,title: ldValue(conceptScheme[DcTerms.title])
        ,terms: {}
    });

    const termsObj = ldNodes
        .filter(ldNode => ldValue(ldNode[JsonLd["@type"]]) === Skos.Concept)
        .reduce((terms, conceptInstance) => {
            // Map skos:Concept onto our 'Term' import format type
            let aliases = ldValue(conceptInstance[Skos.altLabel]) || [];
            if (aliases && !Array.isArray(aliases)) {
                aliases = [aliases];
            }
            const termUri = ldValue(conceptInstance[JsonLd["@id"]]);
            const term = {
                type: "Term"
