...
async function parseCsvToJson(content, fileName, glossConf) {
    const csvParserModule = await csvParserModuleLoaded;
    const {uri, import:importConf } = glossConf;
    const {title, dialect, schema} = importConf || {};
    const {delimiter, quoteChar, escapeChar} = dialect || {};
    if (! csvParserModule) {
        ...
        return new Promise((resolve, reject) => {
            ...
            // Assume to have headers embedded if there aren't field names configured.
            let hasHeader = fieldNames.length > 0 ? fieldNames : true;
