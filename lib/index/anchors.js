...
const {getIndex, byGroupHeading} = require("../indexer");
const SortedMap = require("collections/sorted-map");
const SortedSet = require("collections/sorted-set");

const api = {};
const IDX_ANCHORS = "anchors";
...
api.getAST = function(context, configOpts) {
    ...
    const anchorSet = selectAnchorsFromIndex(className);

    let tree = [
        heading(1, text(title || toFirstUpper(className)))
    ];
    if (! groupByHeadingDepth || groupByHeadingDepth < 0) {
        tree.push(getListOfAnchorsAst(context, anchorSet, configOpts));
    } else {
        tree.push(getAnchorsBySectionAst(context, anchorSet, configOpts));
    }
    return root(tree);
};
 ...
 * @param {IndexEntry[]} anchorSet
 */
function getAnchorsBySectionAst(context, anchorSet, configOpts) {
    return paragraph(
        anchorSet.group(byGroupHeading)
            .map(group => group[1])
            .map(anchors => {
...
function selectAnchorsFromIndex(anchorClass) {
    const anchors = getIndex(IDX_ANCHORS) || SortedMap();
    return anchors.get(anchorClass) || SortedSet();
}
