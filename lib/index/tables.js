...
api.indices = [
    {
        ...
        ,filterFn: (indexEntry) => {
            ...
            if (node.type === "table") {
                ...
                if (text && text.match(TABLE_LABEL_REGEXP2)) {
                    return true;
                }
            }
        }
...
function selectTablesFromIndex() {
    ...
    let tables = [
        ...Object
            .keys(tableEntries)
            .filter(key => tableEntries[key]                       // [1]
                .find(entry => entry.node.type === "table")
            )
...
function mergeTableWithLabel(indexEntries) {
    ...
    let description = "";
    if (htmlEntry) {
        // Derive table description from preceding html comment
        const matches = htmlEntry.node.value.match(TABLE_LABEL_REGEXP);
        if (matches.length > 1) {
            description = matches[1];
        }
    } else if (textEntry) {
        // Derive table description from preceding paragraph
    ...
    if (! description) {
        // Derive table description from table headings
        description = getTableHeaders(tableEntry.node)
            .filter(text => !!text)
            .join(" | ");
    }
    if (! description) {
        // Use preceding section heading to name the table
        description = getNodeText(tableEntry.headingNode);
    }
    if (! description) {
        // Use file name if none of previous attempts succeeded
        description = `${tableEntry.file}`;
    }
    return {
        ...tableEntry
        , description
