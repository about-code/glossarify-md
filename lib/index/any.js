...
api.indices = [
    {
        ...
        ,keyFn: (indexEntry) => {
            const htmlNodeValue = indexEntry.node.value;
...
function getListOfAnchorsItemAst(context, indexEntry, configOpts) {
    ...
    if (idMatches) {
        ...
        anchor = anchorId.join("-");
    }
    // try anchor from closest heading or none at all
    if (! anchor && indexEntry.headingNode) {
    ...
    if (! label) {
        label = getHtmlNodeInnerText(indexEntry.node, indexEntry.parents);
    }
