...
,{
    id: api.IDX_TERMS_BY_GLOSSARY
    , title: "Terms by Glossary"
    , filterFn: (node) => node.type === TermOccurrenceNode.type
    , keyFn: (node) => node.termDefs[0].glossary.file
    , conf: (context) => null
}];
...
api.getAST = function (context, indexFileConf) {
    ...
    if (indexFileConf.glossary) {
        indexDescriptor = api.indexes.find((i) => i.id === api.IDX_TERMS_BY_GLOSSARY);
        indexEntries = getIndex(indexDescriptor.id);
        indexEntries = { 0: indexEntries[indexFileConf.glossary] }
    } else {
        indexDescriptor = api.indexes.find((i) => i.id === api.IDX_OCCURRENCES_BY_TEXT)
        indexEntries = getIndex(indexDescriptor.id);
    }
