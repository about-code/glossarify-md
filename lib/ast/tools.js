...
/**
 * Tests whether the given node follows a node of `prevType`.
 * If the immediate predecessor is a paragraph then it tests
 * for the last child in the paragraph (ignoring the paragraph, therefore "near").
 *
 ...
 * @param {number} [index]
 */
api.followsOnType = function followsOnType(prevType, node, parent, index) {
    let children = [];
    if (! parent) {
        return false;
    } else {
    ...
    if (index === undefined || index === null || index < 1) {
        index = children.findIndex(item => item === node);
    }
    if (index >= 1) {
        const prev = children[index-1];
        if (prev.type === prevType) {
            return true;
        } else if (prev.type === "paragraph") {
            return !!api.getLastChildByType(prev, prevType);
        }
    }
    return false;
};
