...
import { IndexEntry } from "./model/index-entry.js";
import { getVFilePath, relativeFromTo } from "./path/tools.js";
import { collator } from "./text/collator.js";
import { pad } from "./text/tools.js";

/**
 * Generic structure of index entries. Structure of index entry candidates
 * prior to being selected for indexing.
 * @typedef {{
 *    node: Node,
 *    headingNode: Node,
 ...
 */
/**
 * Value object which describes an Index as an index key provider function
 * and a filter function. Both functions will receive an IndexEntry
 * candidate object. The key function determines a particular property
 * accessible via the IndexEntry candidate as the one whose property values
 * should be used as lookup keys. The filter function determines whether a
 * particular IndexEntry candidate should be grouped under the index key.
 *
 * For indexing nodes of abstract syntax trees the IndexEntry candidates
 * provide access to an AST node.
 *
 * @typedef {{
 *    id: string,
 *    keyFn: (entry: IndexEntry) => string|number
 ...
 * tree visitor for indexing AST nodes based on index definitions
 * passed to the factory.
 *
 * @param {Context} context
 * @param {IndexDef[]} indexDefs Array with index definitions
 * @returns {(tree: Node, vFile) => Node} mdast transformer
 */
export function withIndexer(context, indexDefs) {
    if (context !== _runContext) {
        reset(context); // Watch mode: reset module-internal contextual state.
    }
    return () => (tree, vFile) => {
        visit(tree, getNodeVisitor(context, indexDefs, vFile));
        return tree;
    };
}
 ...
 */
function getNodeVisitor(context, indexConfs, vFile) {
    ...
    return function visitor(node, idx, parent) {
        ...
        // Indexing...
        for (let i = 0, len = indexConfs.length; i < len; i++) {
            const idx = indexConfs[i];
            const id = idx.id;
            if (! _index[id]) {
                _index[id] = {};
            }
 ...
 * `headingNode`.
 *
 * @param number groupByHeadingDepth
 */
 ...
 * @return {{ key: IndexEntry<TermDefinitionNode>[] }}
 */
