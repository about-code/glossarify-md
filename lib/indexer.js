...
api.indexer = function(opts) {
    return (tree, vFile) => {
        uVisit(tree, getNodeVisitor(opts, vFile));
        return tree;
    };
};
...
function getNodeVisitor(args, vFile) {
    const {context} = args;
    const searchExprFlags = `u${context.opts.ignoreCase ? "i": ""}`;
    ...
    return function visitor(node, idx, parent) {
        ...
        for (let i = 0, len = indices.length; i < len; i++) {
            ...
            if (idx.filterFn.call(null, node, vFile, context)) {
                ...
                let values = _index[id].get(key);
                if (! values) {
                    values = SortedSet();
                    _index[id].set(key, values);
                }
                values.push(
                    new IndexEntry({
                        id: _indexEntryId
 ...
 */
api.getIndex = function(indexConfOrId) {
    let id = indexConfOrId;
    if (indexConfOrId && typeof indexConfOrId === "object") {
        id = indexConfOrId.id;
    }
    if (Object.prototype.hasOwnProperty.call(_index, id)) {
        return _index[id];
    }
};
