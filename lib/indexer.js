...
api.indexer2 = function(opts) {
    return (tree, vFile) => {
        const documentFilename = `${vFile.dirname}/${vFile.basename}`;
        uVisit(tree, getNodeVisitor2(index2.nodes, index2.definitions, documentFilename));
        return tree;
    };
};

api.getNodeIndex = function() {
    return index2.nodes;
}

api.getDefinitionIndex = function() {
    return index2.definitions;
}
 ...
 * @returns {(node: Node) => void}
 */
function getNodeVisitor2(nodeIndex, defIndex, file) {

    // Headings in the current document file.
    // headings[depth][headings]
    const headings = [[], [], [], [], [], [], [], [], [], []];
    let headingVisitedLast = null;
    function getHeadingVisitedLast(node) {
        if (node.type === "heading" && node.depth <= 9) {
            headings[node.depth].unshift(node);
            node.parentHeading = headings[node.depth-1][0];
            headingVisitedLast = node;
        }
        return headingVisitedLast;
    }

    return function visitor(node) {
        const headingNode = getHeadingVisitedLast(node);
        ...
        if (node.type === "definition") {
            defIndex[`${file}#${node.identifier}`] = node;
        }
    };
}
