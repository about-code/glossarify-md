 ...
 * @param {Glossary} glossaryVFile
 */
function getNodeVisitor(context, glossaryVFile) {
    ...
    const {baseUrl, indexing} = context.conf;
    const headingDepths = indexing.headingDepths
    const termHint = glossaryVFile.termHint;
    const vocabUri = glossaryVFile.uri || baseUrl || "#";
    ...
    return function visitor(node) {
        if (node.type === "heading") {
            ...
            if (headingDepth === 1) {
                glossaryVFile.title = getNodeText(node);
            }
            if (headingDepths[headingDepth] === true) {
                ...
                termNode = new TermDefinitionNode({
                    uri: concatUrl(vocabUri || baseUrl || "", "", headingId)
                    ,value: term
                    ,glossary: glossaryVFile
                    ,hint: termHint
                    ,headingId: headingId
