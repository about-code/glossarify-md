...
function getTermUri(context, glossary, headingId) {
    ...
    let termLink = getFileLinkUrl(context, outDir, glossary.path, headingId);
    if (linking.paths === "relative") {
        termLink = termLink.replace("./", vocabUri);
    } else if (linking.paths === "absolute") {
        termLink = termLink.replace(baseUrl, vocabUri);
    }
    return termLink;
}
