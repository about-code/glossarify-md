...
function getTermVisitor(context, glossary) {
    return function visitor(node, index, parent) {
        ...
        if (node.type === "heading") {
            ...
            addNewTerm(node.value, terms, parent.url, glossary)
            ...
            const currTerm = terms[0];
            if (/\<\!--\n?\s?Aliases:/g.test(node.value)) {
                currTerm.aliases = node.value
                    .replace(/\<\!--\s?\S?Aliases:\s?\n{0,}/gs, "")
                    .replace(/\s?\S?--\>/gs, "")
                    .replace(/\b(.*)\b/gs, "$1")
                    .split(/\s?,\s?/);
            }
            let regExp = "\\b" + escapeRegExp(currTerm.term);
            currTerm.aliases.forEach((a) => {
                regExp += "|" + escapeRegExp(a)
            });
            regExp += "\\b";
            currTerm.regex = new RegExp(regExp);
...
function addNewTerm(text, terms, url, glossary) {
    const term = new Term({
        glossary: glossary,
        hint: glossary.termHint,
        anchor: url,
        term: text,
        regex: new RegExp("\\b" + escapeRegExp(text) + "\\b")
    });
