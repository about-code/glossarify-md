...
api.readTermDefinitions = function(context) {
    const {baseDir, outDir, glossaries, excludeFiles} = context.opts;
...
function getTermVisitor(context, glossary) {
    return function visitor(node, index, parent) {
        ...
        if (node.type === "heading") {
            ...
            addNewTerm(terms, node.value, parent.url, glossary, opts.ignoreCase);
...
function addNewTerm(terms, text, url, glossary, ignoreCase) {
    const term = new Term({
        glossary: glossary,
