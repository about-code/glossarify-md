...
import remark_parse_pandoc_heading_id from "remark-heading-id";
...
import { linker, linker_heading_auto_id } from "./linker.js";
...
export function readGlossaries(context) {
    const {
        baseDir, outDir, glossaries, keepRawFiles, excludeFiles, linking,
        unified: unifiedConf
    } = context.conf;
    return new Promise((resolve, reject) => {
        const unifiedNginConf = Object.assign({}, unifiedConf, {
            processor: unified()
                ...
                .use(remark_parse_pandoc_heading_id)
                .use(remark_gfm)
                .use(linker_heading_auto_id, { algorithm: linking.headingIdAlgorithm })
...
export function readDocumentFiles(context) {
    const {
        baseDir, outDir, includeFiles, keepRawFiles, excludeFiles, linking,
        dev, unified: unifiedConf
    } = context.conf;
    return new Promise((resolve, reject) => {
        const unifiedNginConf = Object.assign({}, unifiedConf, {
            processor: unified()
