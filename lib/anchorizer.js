...
import { getLastChildByType, getNodeText, getTableHeaders, isHtmlComment } from "./ast/tools.js";
 ...
 * tree visitor for searching the abstract syntax tree for figures and
 * tables and adding HTML anchor elements. The indexer will index HTML
 * elements by their 'class' HTML attribute together with any elements
 * that have been added, manually, by the author.
 *
 * This allows generating a list of indexed HTML elements, e.g. a list
 * of figures and a lists of tables, with links from those list items
 * to the book sections where they have been found.
 *
 * @private
 * @param {Context} context
 */
export function withAnchorizer(context) {
    ...
    if (listOf.length > 0) {
        return () => (tree, vFile) => {
            ...
            visit(tree, getNodeVisitor(context, vFile, slugger, generateFilesConf));
            return tree;
        };
    } else {
        return () => (tree) => tree;
...
function anchorizePattern(node, index, parent, slugger, generateFilesConf) {
    ...
    for (let i = 0, len = listOf.length; i < len; i++) {
        ...
        if (match[1]) {
            ...
            title = match[1];
            id = `${slugger.slug(title.substring(0, 20))}`;
        } else {
