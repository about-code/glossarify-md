...
function getTermsSorted() {
    ...
    for (let i = 0, len = terms.length; i < len; i++) {
        result.push(_byAnchor[terms[i]].sort((indexEntry1, indexEntry2) => TermDefinitionNode.compare(indexEntry1.node, indexEntry2.node)));
    }
    return result;
}
...
api.linker = function(args) {
    ...
    return (tree, vFile) => {
        uVisit(tree, getNodeVisitor(context, vFile, indexEntriesMap));
        return tree;
    };
};
 ...
 */
function getNodeVisitor(context, vFile, indexEntriesMap) {
...
function linkifyAst(paragraphNode, headingNode, indexEntrySet, context, vFile) {
    ...
    const maxReplacements = (context.conf.linking.terms === "first-in-paragraph") ? 1 : Infinity;
    let countReplacements = 0;
    let linkNodes = [];
    if (hasMultipleDefs) {
        // Create superscript links if there exist multiple definitions for the term
        linkNodes = termNodes.map((tNode, j) => {
            const shortDesc = tNode.getShortDescription();
            const toGlossaryPath = tNode.glossary.file;
            const glossUrl = getFileLinkUrl(context, fromDocumentPath, toGlossaryPath, tNode.anchor);
    ...
    const newParagraphNode = findReplace(paragraphNode, termNode.regex, (linkText) => {
        ...
        return new TermOccurrenceNode({
            ...
            ,children: [
                link(
                    getFileLinkUrl(context, fromDocumentPath, toGlossaryPath, termNode.anchor)
