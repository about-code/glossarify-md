...
function getTermOccurrenceVisitor(context, vFile, glossaryStats) {
    ...
    return (termOccurrenceNode) => {
        let termDefs = termOccurrenceNode.termDefs;
        const countAlternativeDefs = termDefs.length - 1
            , maxAlternativeDefs = Math.abs(linking.limitByAlternatives)
            , hasAlternativeDefs = countAlternativeDefs > 0
        ...
        if (hasAlternativeDefs) {
            // In case of occurrences of ambiguous term definitions priortize
            // the definition of the glossary whose unambiguous (other) terms
            // have been mentioned most often within the same file.
            termDefs = termDefs.sort((termDef1, termDef2) => {
        ...
        const termDefinitionNode = termDefs[0];
