...
export function linker(args) {
    ...
    const getTermsSorted = function (_byAnchor) {
        ...
        let terms = Object.keys(_byAnchor);
        let result = [];
        for (let i = 0, len = terms.length; i < len; i++) {
            result.push(_byAnchor[terms[i]]
                .sort((indexEntry1, indexEntry2) => {
                    return TermDefinitionNode.compare(indexEntry1.node, indexEntry2.node);
                }));
        }
        return result;
        // Implementation note:
    ...
    /** @type {{ key: IndexEntry<TermDefinitionNode>[] }} */
    const termsByPhrase = getTermsSorted(getIndex(IDX_TERMS_BY_PHRASE));

    return (tree, vFile) => {
        visit(tree, ["link"], getLinkVisitor(context, vFile, termsById));
        visit(tree, getLinkifyVisitor(context, vFile, termsByPhrase));
        return tree;
    };
}
