...
function getLinkifyVisitor(
    ...
    const isTermContextScopeHeading = /^by-glossary-refCount/.test(termContextKind);
...
function getTermOccurrenceVisitor(context, vFile) {
    ...
    return (node) => {
        if (node.type === TermOccurrenceContextNode.type) {
            ...
            linkify(node, histogram, limitByAlternatives, sortAlternatives);
            return SKIP;
        }
    };
}

function linkify(node, histogram, limitByAlternatives, sortAlternatives) {
    ...
    if (hasAmbiguousDefs && /^by-glossary-refCount/.test(sortAlternatives)) {
    ...
    if (hasAmbiguousDefs && permittedMaxAmbiguousDefs > 0) {
        linkNodes = validDefinitionNodes
            .slice(1, permittedMaxAmbiguousDefs)
