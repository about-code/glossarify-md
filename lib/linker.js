...
api.linker = function(args) {
    ...
    const indexEntriesByAnchor = getIndex("term_defs");
    return (tree, vFile) => {
 ...
 * following rules:
 *
 * 1. Given #custom-heading-id is unique accross documents then a path to the
 *    file declaring #custom-heading-id should be prepended (see example below)
 * 2. Given #custom-heading-id is not unique accross documents then local
 *    declaration should take precedence over external declaration.
 * 3. Given #custom-heading-id is not unique accross documents and there is no
...
function linkifyAst(paragraphNode, headingNode, indexEntrySet, context, vFile) {
    ...
    const maxReplacements = (context.conf.linking.mentions === "first-in-paragraph") ? 1 : Infinity;
