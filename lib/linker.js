...
const {IDX_TERM_DEFINITIONS_BY_TERM} = require("./index/terms");
...
api.linker = function(opts) {
    const {context} = opts;
    const indexMap = getIndex(IDX_TERM_DEFINITIONS_BY_TERM);
    registerNodeTypes(this.Compiler);
    return (tree, vFile) => {
        uVisit(tree, getNodeVisitor(context, vFile, indexMap));
        return tree;
    };
};
 ...
 * @param {{context: Context}} opts
 */
function getNodeVisitor(context, vFile, indexMap) {
    let headingNode = null;
    return (node) => {
        if (node.type === "paragraph" || node.type === "tableCell") {
            for (const indexEntrySet of indexMap.values()) {
 ...
 */
function linkifyAst(paragraphNode, headingNode, termDefSet, context, vFile) {
    const hasMultipleDefs = termDefSet.length > 1;
    const termNode = termDefSet.one().node;
    const newLinkNodes = [];
    ...
    for (let i = 0, len = newLinkNodes.length; i < len; i++) {
        ...
        if (lNodePos >= 0) {
            newParagraphNode.children = pChildren
                ...
                .concat(termDefSet.map((indexEntry, j) => {
                    const termNode = indexEntry.node;
                    const toGlossaryPath = termNode.glossary.file;
                    const glossUrl = getFileLinkUrl(context, fromDocumentPath, toGlossaryPath, termNode.anchor);
                    const shortDesc = termNode.getShortDescription();
