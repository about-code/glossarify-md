...
function getSearchVisitor(
    ...
    const sortAlternativesIsByRefCount = REGEXP_SORT_ALTERNATIVES_BY_REFCOUNT.test(sortAlternativesMethod);
    const refCountEvalScope = parseInt(sortAlternativesMethod.slice(-1)) || 0;
    let currentRefCountContext = termContextTree;
    ...
    return (node, index, parent) => {
        ...
        if (node.type === "paragraph" || node.type === "tableCell") {
            for (let term_i = 0, len = termDefinitions2D.length; term_i < len; term_i++) {
                ...
                findTermOccurrencesByPhrase(node, headingNode, termDefinitionNodes, context, vFile, currentRefCountContext);
            }
            ...
            if (sortAlternativesIsByRefCount && node.depth <= refCountEvalScope) {
                const newRefCountContext = new TermOccurrenceContextNode();
                currentRefCountContext.children.push(newRefCountContext);
                currentRefCountContext = newRefCountContext;
            }
            headingNode = node;
            return SKIP;
            ...
            findTermOccurrenceById(node, headingNode, termIndexEntriesById, context, vFile, currentRefCountContext, index, parent);
            return SKIP;
...
function getTermOccurrenceVisitor(context, vFile) {
    ...
    return (node) => {
        if (node.type === TermOccurrenceContextNode.type) {
            ...
            linkify(node, histogram, limitByAlternatives, sortAlternativesMethod, context, vFile);
            return SKIP;
        }
    };
}

function linkify(node, histogram, limitByAlternatives, sortAlternativesMethod, context, vFile) {
