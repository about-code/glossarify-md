...
import { classifyAncestry, getFileLinkUrl, isParentOrChildOrSibling } from "./path/tools.js";
/**
...
function linkifyAst(paragraphNode, headingNode, termDefinitionNodes, context, vFile) {
    const linking = context.conf.linking
        ...
        , skipOnNonAncestor = linking.limitToAncestry
        , maxReplacements = (linking.mentions === "first-in-paragraph") ? 1 : Infinity
        , termDefinitionNode = termDefinitionNodes.find(node => {
            // termDefinitionNodes includes any indexed nodes but not all
            // indexed nodes are to be included in linking
            // (see option linking.headingDepths)
            return !! linking.headingDepths[node.headingDepth];
        })
        ;
    ...
    if (skipOnNonAncestor && !isParentOrChildOrSibling(termDefinitionNode.glossVFile.path, vFile.path)) {
        return
    }
        ...
        linkNodes = termDefinitionNodes
            .filter((node, count) => {
                return node !== termDefinitionNode
                    && linking.headingDepths[node.headingDepth]
...
function resolveGlossaryUrl(context, vFile, termDefinitionNode) {
    ...
    if (glossVFile.glossConf.linkUris && termDefinitionNode.uri) {
        ...
        const classifiedAncestry = classifyAncestry(fromDocumentPath, toGlossaryPath);
