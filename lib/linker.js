...
export function withLinker(context) {
    ...
    return () => {
        return (tree, vFile) => {
            ...
            /** @type { TermOccurrenceNode[] } */
            const termOccurrencesFound = [];

            /** @type { Function } */
            const termOccurrenceVisitor = getTermOccurrenceVisitor(context, vFile, glossaryStats)
                , linkifyVisitor = getLinkifyVisitor(
                    context
                    , vFile
                    , termDefinitions2D
                    , termIndexEntriesById
                    , termOccurrencesFound
                    , glossaryStats
                );

            visit(tree, linkifyVisitor);
            for (let i, len = termOccurrencesFound.length; i < len; i++) {
                termOccurrenceVisitor(termOccurrencesFound[i]);
            }
            return tree;
        };
    };
    ...
    return (node, index, parent) => {
        if (node.type === "paragraph" || node.type === "tableCell") {
            for (let term_i = 0, len = termDefinitions2D.length; term_i < len; term_i++) {
                ...
                findTermOccurrencesByPhrase(node, headingNode, termDefinitionNodes, context, vFile, termOccurrencesFound, glossaryStats);
            }
            ...
            findTermOccurrenceById(node, headingNode, termIndexEntriesById, context, vFile, termOccurrencesFound, glossaryStats, index, parent);
            return SKIP;
        } else {
            return CONTINUE;
        }
