...
import { IDX_TERMS_BY_ANCHOR } from "./index/terms.js";
...
export function linker(args) {
     ...
     */
    const getTermsSorted = function (_byAnchor) {
    ...
    const indexEntriesByAnchorSorted = getTermsSorted(indexEntriesByAnchor);
    return (tree, vFile) => {
        visit(tree, ["link"], getLinkVisitor(context, vFile, indexEntriesByAnchor));
        visit(tree, getLinkifyVisitor(context, vFile, indexEntriesByAnchorSorted));
        return tree;
    };
}
 ...
 */
export function linker_heading_auto_id(args) {
        ...
        visit(tree, "heading", (node) => {
            ...
            if (node.data && node.data.id) {
                ...
                id = crypto.createHash(algorithm)
                    .update(getNodeText(node) + vFile.path)
                    .digest("hex")
                    .toString()
                    .substr(0,7);
            } else {
...
function getLinkVisitor(context, vFile, indexEntriesMap) {
    return (lNode) => {
        ...
        const termDefinitionIndexEntry = indexEntriesMap[anchor];
    ...
    return (node) => {
        if (node.type === "paragraph" || node.type === "tableCell") {
            for (let i = 0, len = indexEntriesMap.length; i < len; i++) {
                const termDefinitions = indexEntriesMap[i].map(e => e.node);
                linkifyAst(node, headingNode, termDefinitions, context, vFile);
            }
...
function linkifyAst(paragraphNode, headingNode, termDefinitionNodes, context, vFile) {
    const linking = context.conf.linking
        ...
        , termDefinitionNode = termDefinitionNodes[0]
        , countDefinitions = termDefinitionNodes.length
    ...
    if (linking.headingDepths[termDefinitionNode.headingDepth] !== true) {
        return;
    }
