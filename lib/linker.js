...
const {TermOccurrence} = require("./ast/with/term-occurrence");
...
function getTermsSorted() {
    ...
    for (let i = 0, len = terms.length; i < len; i++) {
        result.push(_byAnchor[terms[i]].sort((indexEntry1, indexEntry2) => TermDefinition.compare(indexEntry1.node, indexEntry2.node)));
    }
    return result;
}
