 ...
 * @returns
 */
export function getUrlPath(baseDir, targetFile) {
    if (targetFile instanceof VFile === false) {
        targetFile = new VFile({ path: targetFile });
    }
    const from_ = toForwardSlash(baseDir);
    const to_   = toForwardSlash(targetFile.path || "");
    return path
        .resolve(from_, to_ || "")
        .replace(from_, "")
