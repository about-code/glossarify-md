...
export function resolveGlossaryUrl(context, vFile, termDefinitionNode) {
 ...
 */
export function readTextFile(textFileNameOrUrl, baseDir, contentTypeMap = {}) {
    const fileScheme = "file://";
    const httpScheme = "http://";
    const httpsScheme = "https://";
    if (textFileNameOrUrl.match(new RegExp(`^(${httpsScheme}|${fileScheme})`))) {
        // https://
        return readTextFileUrl(textFileNameOrUrl, contentTypeMap);
    } else if (textFileNameOrUrl.substring(0, httpScheme.length) === httpScheme) {
        // http://
        ...
        return new Promise((resolve, reject) => {
            const cb = (err, content) => {
                if (err) {
                    ...
                    const matches = textFileNameOrUrl.match(/\.([a-zA-Z]*)$/);
                    const fileExt = matches[1] || "txt";
                    const contentType = contentTypeMap[fileExt] || "text/plain";
                    resolve({ contentType, content });
                }
            };
            if (textFileNameOrUrl.substring(0, fileScheme.length) === fileScheme) {
 ...
 */
function readTextFileUrl(jsonFileUrl, contentTypeMap) {
    return new Promise((resolve, reject) => {
         ...
         * ~~~
         */
        const contentTypes = Object.values(contentTypeMap);
