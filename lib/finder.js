 ...
 * searches for terms from glossaries and inserts new TermOccurrenceNode
 * types where terms have been found. The resulting AST can then be
 * processed with term occurrences being represented as distinct nodes in
 * the tree.
 *
 * @private
    ...
    return () => {
        return (tree, vFile) => {
            ...
            visit(tree, FileNode.type, (fileNode) => {
                const metaTree = new TermOccurrenceContextNode();
                FileNode.addMetaTree(fileNode, metaTree);

                visit(fileNode, getVisitor(context, vFile, termDefinitions2D, termIndexEntriesById, metaTree));
            });
            return tree;
        };
    };
...
function getVisitor(
    ...
    // While searching for term occurrences collect metrics on the number
    // of glossary references which can be used for context-sensitive
    // disambiguation. Create a separate refCount evaluation context per file (0)
    // or section (1-6) depending on config 'linking.sortAlternatives.by'.
    const sortAlternativesMethod = context.conf.linking.sortAlternatives.by;
    const sortAlternativesByRefCount = REGEXP_SORT_ALTERNATIVES_BY_REFCOUNT.test(sortAlternativesMethod);
    const refCountEvalScope = parseInt(sortAlternativesMethod.slice(-1)) || 0;
    let currentTermContextSubtree = termContextTree;
    ...
    return (node, index, parent) => {
        ...
        if (node.type === "paragraph" || node.type === "tableCell") {
            for (let term_i = 0, len = termDefinitions2D.length; term_i < len; term_i++) {
                ...
                findTermOccurrencesByPhrase(node, headingNode, termDefinitionNodes, context, vFile, currentTermContextSubtree);
            }
            ...
            if (sortAlternativesByRefCount && node.depth <= refCountEvalScope) {
                const newRefCountContext = new TermOccurrenceContextNode();
                currentTermContextSubtree.children.push(newRefCountContext);
                currentTermContextSubtree = newRefCountContext;
            }
            headingNode = node;
            return SKIP;
            ...
            findTermOccurrenceById(node, headingNode, termIndexEntriesById, context, vFile, currentTermContextSubtree, index, parent);
            return SKIP;
