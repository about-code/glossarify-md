...
const {toSystemSlash} = require("./pathplus");
const {getAST} = require('./indexer');
...
api.writeIndex = function(context) {
    ...
    if (! indexFile) {
        ...
        return this.writeMarkdownFile(context, getAST(context), indexFile.file);
    } else {
        return this.writeMarkdownFile(context, getAST(context), indexFile);
    }
}
