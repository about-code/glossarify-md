...
function redactPaths(context) {
    ...
    terms.byOccurrence().forEach((term) => {
        ...
        const paths = Object.keys(occurrences).sort();
        const newPaths = {};
        for (i = 0, len = paths.length; i < len; i++) {
            const oldPath = paths[i];
            const newPath = toForwardSlash(toRedactedPath(paths[i]));
            newPaths[newPath] = occurrences[oldPath];
        }
        term.occurrences = newPaths;
    });
}
