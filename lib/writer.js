...
const {getAST: getIndexerAST, IDX_TERM_DEFINITIONS} = require("./index/terms");
...
api.writeReport = function(context) {
    ...
    const termsIndexed = getIndex(IDX_TERM_DEFINITIONS).get(0);
...
api.writeTestOutput = function (context) {
    ...
    const indexedTerms = getIndex(IDX_TERM_DEFINITIONS).get(0);
    const promises = [];
    if (termsFile && indexedTerms) {
        // Important! Write terms in defined order to get reliable diff.
        const termNodes = indexedTerms.map(indexEntry => indexEntry.node);
        promises.push(writeTextFile(context, termsFile, JSON.stringify(termNodes, null, 2)));
    }
    if (reportsFile) {
